...
| :--------------------- | :------------------------------------------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------ | :--------------------------------------------------------------------------------------------------------------------------------------------------- |
| **LoRa (E22-900T22D)** | VCC <br> GND <br> RXD <br> TXD <br> **AUX** <br> **M0** <br> **M1** | **3.3V** <br> **GND** <br> **PA9 (Serial1 TX)** <br> **PA10 (Serial1 RX)** <br> **PA11 (GPIO, INPUT)** <br> **PA12 (GPIO, LOW)** <br> **PA15 (GPIO, LOW)** | UART hatları PA9/PA10 üzerinde tutulur. AUX/M0/M1 kontrolü PA11/PA12/PA15 ile yapılır. |
...
| **ESC (Skywalker 30A V2-UBEC, 4 kablo)** | **Beyaz** (throttle sinyal) <br> **Kırmızı** (+5V BEC) <br> **Siyah** (GND) <br> **Sarı** (Reverse Brake kontrol) | **PA0 (ESC PWM / LY throttle, open-drain + pull-up)** <br> **5V güç hattı** <br> **GND ortak** <br> **Kullanılmıyor (opsiyonel RX kanalına)** | Beyaz kablo ESC gaz sinyalidir ve kumandadaki **LY (throttle)** kanalına karşılık gelir. **PA0 sinyal hattına 4.7kΩ-10kΩ pull-up direnci 5V'a bağlanmalıdır.** Kırmızı/siyah BEC çıkışıdır (servo +5V besleme). Sarı kablo batarya telemetrisi değildir; Reverse Brake ON/OFF kontrol hattıdır. |
...
| **FT232 (USB-UART, opsiyonel)**   | TXD <br> RXD <br> VCC <br> GND                                      | **PB10/PB11 gibi boş bir UART hattı önerilir** <br> **3.3V veya 5V** <br> **GND**                                                                       | PA11/PA12 LoRa mode pinlerinde kullanıldığı için FT232 bu pinlere bağlanmamalıdır. Gerekirse farklı UART pinlerine taşınır. |
...
> ⚠️ **Skywalker 4 Kablo Notu** — Kırmızı kablo ESC'nin BEC 5V hattıdır; **STM32'nin 3.3V pinine bağlanmaz**. Siyah kablo ile STM32 GND ortak edilmelidir. Beyaz kablo PA0'a gider. Sarı kablo Reverse Brake kontrol hattıdır, batarya ölçümü için kullanılmaz.
